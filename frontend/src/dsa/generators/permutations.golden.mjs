import { permutationsEvents } from './permutations.js';
...
function check(input) {
  const events = permutationsEvents(input);
  const frames = compile(input, events);

  // 1) determinism
  const again = permutationsEvents(input);
...
console.log('permutations golden check\n');
