...
NODES = [
    ...
    ("Web Framework & API", "Validation", "Pydantic Models and Validators", "medium", "Declarative schema; field_validator for custom rules."),
    ("Web Framework & API", "Validation", "Config & Pydantic Settings", "easy", "Typed config from .env; never hardcode secrets."),
    ...
    ("Data Layer", "ORM", "Sessions and expire_on_commit", "hard", "expire_on_commit=False to use objects after commit."),
    ("Data Layer", "ORM", "Eager loading (selectinload)", "hard", "Avoid lazy-load on a closed async session (MissingGreenlet)."),
    ("Data Layer", "ORM", "The N+1 Queries Problem", "medium", "Join / batch instead of querying once per row."),
    ("Data Layer", "Schema & Connections", "Alembic migrations", "medium", "Versioned schema changes; never hand-edit prod."),
    ("Data Layer", "Schema & Connections", "Transactions and Atomicity", "medium", "Commit is all-or-nothing; roll back on error."),
    ("Data Layer", "Schema & Connections", "Connection pooling", "medium", "Reuse connections; tune pool size & overflow."),

    # ---------------- 7 · Testing ----------------
    ("Testing", "pytest", "Pytest Basics and Fixtures", "easy", "Arrange/act/assert; fixtures for setup/teardown."),
    ("Testing", "pytest", "Parametrized tests", "easy", "@pytest.mark.parametrize runs one test over many inputs."),
    ("Testing", "Isolation", "Mocking and Patching", "medium", "unittest.mock / monkeypatch to isolate external calls."),
    ...
    ("Testing", "Quality", "Coverage and CI", "easy", "Measure coverage; run the suite in CI."),

    # ---------------- 8 · Production & Deployment ----------------
    ("Production & Deployment", "Config", "Env Vars and Secrets", "easy", "Config lives in the environment, not code; secrets out of the image."),
    ("Production & Deployment", "Observability", "Structured Logging in Prod", "medium", "JSON logs, levels, request IDs."),
    ("Production & Deployment", "Errors", "HTTPException and Error Shape", "easy", "Raise with status + detail; keep one consistent payload shape."),
    ...
    ("Production & Deployment", "Hardening", "Hardening, Health, and Rate Limits", "medium", "/health for probes; verify tokens; rate-limit + explicit CORS allow-list."),
]
