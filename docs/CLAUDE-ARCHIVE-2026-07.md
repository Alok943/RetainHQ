> **FROZEN ARCHIVE (2026-07-10).** This is the full pre-restructure `CLAUDE.md`, kept for historical reference — decisions, gotcha rationale, and the "Done since" build log. Nothing here is maintained. Current truth: root `CLAUDE.md` (rules + routing), `docs/SYSTEM-OVERVIEW.md` (system state), `docs/DECISIONS.md` (decision log).
...
- **Schema changes:** always via Alembic migrations — never hand-edit the live DB. Every new table's migration MUST include `ENABLE ROW LEVEL SECURITY` (no policies): Supabase's PostgREST exposes the public schema to the anon key, and RLS-with-no-policies is what blocks it; the backend is unaffected (connects as `postgres`, the table owner, which bypasses RLS — migration `f8a3b5c2d9e1`).
...
- **Content-build priority (per JD research runs 1–3, `docs/jd-research-run*.md`):** AI Engineering (in build) → **DSA — Algorithms Visualized** (`seed_dsa.py`, in build) → **Data Engineering next** (run 3: DA→DE is underrated — faster than DA→DS, 230k+ open roles, and SQL (already taught) is the #1 DE filter) → ML Fundamentals. DSA is the universal Round-1 gate across ALL target roles incl. GenAI (runs 1+3).
- **Roadmap backlog (candidates to seed, not built yet):** **Data Engineering** (elevated, see above), **Low-Level Design (LLD/OOD)**, **Git & GitHub**, **DSA Blind 75 / patterns**. Others: Behavioral+HR interview, DevOps & Cloud (Docker/CI/CD/K8s/AWS), Linux & Shell, TypeScript, React (deep), API Design, Testing, Machine Learning (Andrew Ng), Deep Learning, GenAI/LLM apps (RAG/agents), Math for ML, Data Analytics, Java for SWE, C++/CP, Resume + Job Hunt. (Build via the `seed_*.py` pattern: idempotent, fixed UUID, `phase`/`tier` nodes.)
- **Career-Paths feature (Phase 2, backed by `docs/jd-research-run3.md`) — v1 SHIPPED:** `/paths` tab (`CareerPaths.jsx` + static `lib/careerPaths.js`) rendering run-3's 10 role transitions grouped by starting role — status/timeline/ROI/blocker/skill-delta + the roadmaps that build each (resolved against the live roadmap list w/ progress; "coming soon" chip for unbuilt ones like ML/Data-Eng/MLOps). Frontend-only/static — no backend. Future: live "% to role X" tracker. Run 3 is role-MOBILITY strategy, NOT lesson content — don't run it through the content pipeline.
...
> **⚠ This section is a frozen historical log (last verified 2026-07-10 — parts are stale).** The **living, verified** system state — architecture, API surface, schema, security posture, plans — is **`docs/SYSTEM-OVERVIEW.md`**. Trust it over anything below; keep IT updated (see the "Living doc" convention), and do NOT append new "Done since" entries here.
...
- **Living doc — `docs/SYSTEM-OVERVIEW.md` is the single source of truth for system state.** Convention: any change that adds/removes an endpoint or router, touches `models/models.py` or adds a migration, adds a service/integration (LLM, email, analytics…), changes deploy topology/env, or moves the security posture MUST update the relevant section of `docs/SYSTEM-OVERVIEW.md` **in the same commit** and add a one-line entry to its Changelog. Claims must be code-verified, not aspirational — when touching prod-state claims (migration head, roadmap counts), verify via the read-only Supabase MCP first. Do NOT log status in CLAUDE.md (that section is frozen) or create parallel status docs; ARCHITECTURE.md/API.md/FLOWS.md are legacy and carry stale banners.
...
- **Deploy:** frontend **Vercel** (root=`frontend/`, `vercel.json` SPA rewrite required), backend **Railway** (root=`backend/`, **Singapore** region for latency). Env on Railway incl. `ADMIN_EMAIL`, CORS allow-list (apex+www), `GROQ_API_KEY`/`GRADER_ENABLED`. Run `alembic upgrade head` (chain: …`a1b2c3d4e5f6` FSRS → `b2c3d4e5f6a7` prereqs → `a3f1c0d4e7b2` roadmap slug → `a4b2e9f1c8d3` node_id → `e7f2a4c9b1d5` review-invariant partial unique indexes). Prod DB verified at `a4b2e9f1c8d3` on 2026-07-02 (slug + node_id ARE applied); `e7f2a4c9b1d5` + `f8a3b5c2d9e1` (RLS) pending.
