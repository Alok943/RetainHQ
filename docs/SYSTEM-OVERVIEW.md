...
## 2. Data model (18 tables; 12 prod-verified + `question_sets`/`push_subscriptions`/`metric_events` + `reviews.duration_ms` pending deploy + `classrooms`/`classroom_members`/`classroom_roadmaps` pending deploy)

`tracks`, `activities` (the FSRS card: `stability`/`difficulty_fsrs` NULL until first graded review; legacy SM-2 columns still written; optional `roadmap_id`/`node_id` links), `reviews` (due/completed + `rating`/`recalled`/`quality` + `ai_*` grader columns + **`duration_ms`** nullable int, card-shown-to-outcome-submitted via client `performance.now()`, server-clamped to (0, 1_800_000]), `feedbacks`, `roadmaps` (**+ `slug`, + `audience` 'career'|'school', + `user_id` NULL=catalog / set=personal syllabus-upload roadmap**), `roadmap_nodes` (self-ref `parent_id` subtopics), `roadmap_node_prerequisites` (directed edges, powers "Why am I stuck?"), `user_progress`, **`user_prefs`** (audience, server-side; + `custom_roadmaps_created` lifetime syllabus-commit counter — never decremented), **`test_attempts`** (JSONB per-question results; `node_title` is the join key), **`reminder_log`** (unique `(user_id, sent_on)` = at-most-once-daily email+push idempotency, one claim gates both channels), **`question_sets`** (persisted LLM question set per card: JSONB `items` [{question, reference_answer}], `depth` 'main'|'deep', `times_used`; reused ≥2 sessions then regenerated; reference answers never leave the server), **`push_subscriptions`** (one row per browser/device Web Push subscription; `endpoint` unique = the upsert key; RLS), **`metric_events`** (generic JSONB event store for heterogeneous/exploratory learning signals — `event_type`+`payload`+optional `entity_id`; RLS; first producer: syllabus commit's `extraction_edit_delta`), **`classrooms`** (2026-07-16 — `teacher_user_id`, `name`, `school_name`, unique `join_code`, `archived_at`; RLS), **`classroom_members`** (unique `(classroom_id, student_user_id)`; `display_name` is student-typed at join, never the Google identity — see §4; RLS), **`classroom_roadmaps`** (which catalog roadmaps a classroom tracks — the scope boundary for every teacher-facing analytics query; RLS), `alembic_version`.

**Migration chain (28):** `c71d8f31ee19` initial → … → `c2f5a9b3d701` ai-grader → `d4e8a1b2c903` reminder_log → `f4a9c2e1b370` roadmap_id → `a1b2c3d4e5f6` FSRS → `b2c3d4e5f6a7` prereqs → `a3f1c0d4e7b2` slug → `a4b2e9f1c8d3` node_id → `e7f2a4c9b1d5` review invariants (partial unique indexes) → `f8a3b5c2d9e1` RLS → `c4d7e9a2b501` audience+user_prefs → `f2b7d3a9c8e4` test_attempts → `a1c5e8f2d7b3` roadmaps.user_id (applied to prod 2026-07-11 via local `alembic upgrade` against the pooler — the migration file was uncommitted until this change, which crash-looped the deploy on an unresolvable revision) → `b3d9f1a4c6e2` question_sets (RLS enabled) → `c8e2a7f5d1b9` user_prefs.custom_roadmaps_created → `d4f6a8c1e3b7` push_subscriptions (RLS) → `e7a9c2f4b6d8` reviews.duration_ms → `f1b3d5e7a9c0` metric_events (RLS) → **`431279836d74` classrooms/classroom_members/classroom_roadmaps (RLS, head; not yet applied to prod — the migration file exists but hasn't been run, auto-applies on next deploy via the Dockerfile's `alembic upgrade head`, or run `alembic upgrade head` locally against the pooler first if you want it live before the next deploy)**.
