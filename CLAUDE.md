...
| `POST /api/activities/` | Create activity (optional `roadmap_id` link); initializes FSRS card and schedules the **first review tomorrow** (or **due now** for the user's first-ever activity = onboarding demo). Returns `review_due_now`. |
...
- **Done since (coding-education pillar — the white-space build):** **FSRS live** (replaced SM-2; `stability`/`difficulty_fsrs`; migration `a1b2c3d4e5f6`). **Dependency graph** — `roadmap_node_prerequisites` (migration `b2c3d4e5f6a7`; 70 edges seeded by title via `seed_python_swe_prereqs.py`), exposed as `prerequisites`/`unlocks`, plus **"Why am I stuck?"** diagnosis (`GET /api/roadmaps/{id}/blockers` → ready-frontier ranked by transitive unlocks; panel in `RoadmapDetail.jsx`). **Node-anchored lesson view** (`LessonView.jsx` @ `/roadmaps/:id/learn/:slug`; a "Learn" affordance on nodes that have content — NOT a separate tab) rendering overview/why/mistakes/recall/practice/challenge/sources. **Client-side Python visualizer** (`CodeTrace.jsx` + `lib/pyodideRunner.js`: lazy Pyodide + `sys.settrace` step scrubber, zero server compute). **Predict-before-reveal "aha moments"** (guess → run in the visualizer → why; output is never stored — runtime is truth). **Content-curation rig** (`content/`): Gemini-3-Pro (25-rule `PROMPT.md`) → ChatGPT check → `validate.py`; one JSON per node, served statically via `sync-content.mjs` (gitignored, `prebuild`-regenerated). Bulk content goes through Gemini/Sonnet, NOT Claude. Phase 1 (Python-for-SWE): 11/32 curated.
...
- **Deploy:** frontend **Vercel** (root=`frontend/`, `vercel.json` SPA rewrite required), backend **Railway** (root=`backend/`, **Singapore** region for latency). Env on Railway incl. `ADMIN_EMAIL`, CORS allow-list (apex+www), `GROQ_API_KEY`/`GRADER_ENABLED`. Run `alembic upgrade head` (DB head = `a1b2c3d4e5f6`).
