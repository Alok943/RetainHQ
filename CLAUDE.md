...
- **Career-Paths feature (Phase 2, backed by `docs/jd-research-run3.md`):** guided multi-roadmap journeys by role transition (Backend→GenAI = #1 ROI; DA→DE; DS→ML) with skill-delta + timeline + blocker. The "job-aligned bundling" differentiator. Run 3 is role-MOBILITY strategy, NOT lesson content — don't run it through the content pipeline.
